...
// gloabl variables
// ANC UST PAIR 
const address = 'terra1gm5p3ner9x9xpwugn9sp6gvhd0lwrtkyrecdn3';
...
//https://fcd.terra.dev/v1/txs?offset=0&limit=100&account=terra1gm5p3ner9x9xpwugn9sp6gvhd0lwrtkyrecdn3&chainId=columbus-4
  // fetch
  ...
  if (count % 10 == 0) {
    ...
    fs.writeFile(`../fcd-data/anc-ust-pair/anc-ust-pair-results-${file_number}.txt`, JSON.stringify(txs_array),
    function(err) {
      if (err) {
        console.error('error');
      }
